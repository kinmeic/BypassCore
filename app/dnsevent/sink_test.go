...
func TestSinkEmitsUnixDatagram(t *testing.T) {
	...
	defer sink.Close()
	sink.Emit(appdns.Result{Domain: "example.com", IPs: []bcnet.IP{bcnet.ParseAddress("192.0.2.1").IP()}, TTL: 60, ServerTag: "direct", At: time.Unix(123, 0)})
	...
	if event.Domain != "example.com" || len(event.IPs) != 1 || event.IPs[0] != "192.0.2.1" || event.ServerTag != "direct" {
